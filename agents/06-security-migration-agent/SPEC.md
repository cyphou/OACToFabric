...
| File | Purpose |
|------|--------|
| `src/agents/security/security_agent.py` | SecurityMigrationAgent class |
| `src/agents/security/role_mapper.py` | OAC app roles → PBI RLS + Fabric workspace roles |
...
- Do NOT modify TMDL tables/measures, report visuals, or schema DDL
- Do NOT modify discovery or extraction logic
- Only produces RLS/OLS definitions and workspace role mappings
- Security lookup table DDL must be generated by Schema Agent (02) — only provide the spec
- All credential handling must go through `src/core/keyvault_provider.py`

## 1.3 Delegation Guide

| If you encounter… | Delegate to |
|--------------------|-------------|
| Security lookup table DDL creation | **Schema (02)** |
| Init block SQL requires PL/SQL translation | **ETL (03)** |
| RLS role entry needed in TMDL | **Semantic Model (04)** — TMDL roles.tmdl |
| Azure AD group provisioning | **Orchestrator (08)** — external API call |
